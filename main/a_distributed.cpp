...
void renderCornellbox(float scale, const char* filename, Camera* cam, Material* sphereMaterial, Material* floorMaterial, int numSamples=1) {
    ...
    Texture* orangetex = new ConstantTexture(RGBColor(1.f, 0.65f, 0.f)* 35.0f);
    ...
    //Point center0 = Point(150.0f, 100.0f, 240.0f) * scale;
    //Point center1 = center0 + Point(0, random(0,0.1), 0);
    //float t1 = 0.1 * scale, t0 = 0.0f;
    //scene->add(new mSphere(center0, center1, 99.0f*scale, t0, t1, nullptr, sphereMaterial));
    ...
    //new Sphere(Point(150.0f, 100.0f, 240.0f) * scale, 99.0f * scale, nullptr, sphereMaterial)
    //Sphere* light = new Sphere(Point(213 * scale, 549.99f * scale, 227 * scale), 40.0f * scale, nullptr, lightsource);
    AreaLight als(light);
...
void a_distributed() {
    float scale = 0.001f;
    float t1 = 0.1 * scale, t0 = 0.0f;
    PerspectiveCamera* cam = new PerspectiveCamera(Point(0.278f, 0.273f, -0.800f), Vector(0, 0, 1), Vector(0, 1, 0), 0.686f, 0.686f, t0, t1);
